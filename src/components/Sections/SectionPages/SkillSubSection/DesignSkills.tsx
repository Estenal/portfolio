...
const DesignSkills: React.FC = () => {
  ...
  const renderVerticalProgressBar = (
    ...
    <div
      ...
      onClick={onClick}
    >
      <div className="absolute top-8 left-2/3 rotate-85 -translate-x-1/2 bg-rose-600 text-white text-xs font-black px-2 py-1 rounded border-2 border-slate-950 pointer-events-none opacity-0 scale-75 group-hover/bar:opacity-100 group-hover/bar:scale-100 transition-all duration-150 z-50 shadow-[2px_2px_0px_0px_rgba(0,0,0,0.5)] whitespace-nowrap">
    ...
    <section className="p-6 flex flex-col w-full bg-sky-50 text-slate-900 font-sans gap-8 pb-4">
      ...
      <div className="space-y-4 pb-6 border-b-4 border-dashed border-slate-300">
        ...
        <div className="flex items-center justify-end gap-3">
          <span className="text-xl absolute mb-5 -rotate-3 font-black italic text-rose-600 bg-rose-100/50 border-2 border-slate-950 px-2 py-0.5 rounded-md hover:scale-105 z-10">
        ...
        <div className="bg-rose-100 border-2 border-slate-950 p-3 rounded-lg shadow-[2px_2px_0px_0px_rgba(15,23,42,1)] relative -rotate-1 my-4">
           <h4 className="text-sm font-black text-rose-800 italic flex items-center gap-2">
        ...
        <div className="space-y-3 pl-1">
          {category.overview.map((desc, i) => (
            <p key={i} className="text-sm font-bold text-slate-700 leading-relaxed relative pl-4 before:content-['➢'] before:absolute before:left-0 before:text-rose-500">
              {desc}
            </p>
          ))}
        </div>
